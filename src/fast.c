...
void fast_encode_2perm(const perm_t in_p[2], uint8_t *code) {
    for (int i = 0; i < ((2 * PARAM_N1) / 2); i++) {
        uint16_t z2_pi0 = in_p[(i * 2 + 0) / PARAM_N1][(i * 2 + 0) % PARAM_N1];
        uint16_t z2_pi1 = in_p[(i * 2 + 1) / PARAM_N1][(i * 2 + 1) % PARAM_N1];
        store_2_perm_coeff_to_bytearray(code, i, z2_pi0, z2_pi1);
    }
}
...
void fast_decode_2perm(uint8_t *code, perm_t out_p[2]) {
    for (int i = 0; i < ((2 * PARAM_N1) / 2); i++) {
        ...
        load_2_perm_coeff_from_bytearray(&z2_pi0, &z2_pi1, code, i);
        out_p[(i * 2 + 0) / PARAM_N1][(i * 2 + 0) % PARAM_N1] = z2_pi0;
        out_p[(i * 2 + 1) / PARAM_N1][(i * 2 + 1) % PARAM_N1] = z2_pi1;
    }
}
